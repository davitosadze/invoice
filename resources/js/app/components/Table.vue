	...
	export default {
		...
		setup (props) {
			let is_table_advanced = props.setting.is_table_advanced ? [{
						...
						filter: false, 
						cellStyle: { textAlign: 'center'}, 
						cellRenderer: actionCellRenderer, editable: false, colId: 'action' 
					}] : [{ 
						headerName: 'ქმედება', 
						headerClass: 'text-center', 
						maxWidth: 100, 
		...
		methods: {
			...
			removeRequest (id, setting, callback) {
			  ...
			  .catch(function (e) {
			    if (e.response && e.response.statusText) this.$toast.error(e.response.statusText, { position: 'top-right', duration: 7000 })
			  });
			},
			...
			onCellClicked(params) {
				...
				if (params.column.colId == "action") {
					...
					if (action === 'edit') { // params.event.target.dataset.action
						...
						if (action === 'gadawera') { // params.event.target.dataset.action
						let session = [{...params.data}].map(({ purchaser, purchaser_id, special,...rest }) => rest).find((_, index) => index == 0);
					...
					if (action == 'excel') {
						window.location.href = this.setting.url.nested.excel.replace("__id__", params.data.id)
					}

					if (action == 'pdf') {
						window.open(this.setting.url.nested.pdf.replace("__id__", params.data.id), '_blank')
					}
				}
